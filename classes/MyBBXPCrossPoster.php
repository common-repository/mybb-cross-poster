...
class MyBBXPCrossPoster {
	...
	function cross_post($wpid) {
		...
		if ($_POST['mybbxp_content'] == null || $_POST['mybbxp_content'] == "") {
			$this->messageHandler->error('You need to enter your cross-post into the "MyBB Cross-Poster Post"-Box, please do so and update your post.', __FILE__, __LINE__);
			return;
		}
		if ($_POST['mybbxp_title'] == null || $_POST['mybbxp_title'] == "") {
			$this->messageHandler->error('You need to enter your cross-post title into the "MyBB Cross-Poster Post"-Box, please do so and update your post.', __FILE__, __LINE__);
			return;
		}
		...
		$cross_post_content = preg_replace('/!BEGINLINK!(.*)!ENDLINK!/', '[url=' . get_permalink($post->ID) . ']$1[/url]', $cross_post_content);
		$cross_post_content = preg_replace('/!EXCERPT!/', getExcerpt($post->post_content,0,335), $cross_post_content);
		$cross_post_content = preg_replace('/!POST!/', $post->post_content, $cross_post_content);
		$cross_post_content = preg_replace('/!TITLE!/', $post->post_title, $cross_post_content);

        // Remove shortcodes
        $cross_post_content = preg_replace_callback('/\[.*?\]/', function ($match) {
            // ignore BB-Code
            if (in_array(strtolower($match[0]), array('[quote]', '[/quote]', '[img]', '[/img]', '[/url]')) ||
                strrpos(strtolower($match[0]), '[url=', -strlen($match[0])) !== false) {
                return $match[0];
            }
            return '';
        }, $cross_post_content);
		...
		// Remove &nbsp;
		$cross_post_content = preg_replace('/\&.*?\;/', '', $cross_post_content);
		...
		if (empty($mpid)) { // no cross post exists --> create new thread
			...
			$response = wp_remote_post($url, array('body' => $post_data, 'cookies' => $cookies, 'timeout' => 20));
			...
			if ($row->subject == $cross_post_title && $row->message == $cross_post_content) {
				...
				$sql = "select pid"
					. " from " . $options['db_prefix'] . "posts"
					. " where pid=%d;";
				$pid = $this->mybbdb->get_var($this->mybbdb->prepare($sql, $mpid));
				if ($this->there_are_db_errors()) {
					return;
				}
				if ($pid == null) {
					$this->messageHandler->error("Unable to update the cross-post because I can't seem to get the pid from the database. This can happen when you delete the cross-post. Please go to the MyBBXP plugin settings and unlink the crosspost for post-id: " . $wpid . ", then update this post to recreate the cross-post", __FILE__, __LINE__);
					return;
				}
				...
				$response = wp_remote_post($url, array('body' => $post_data, 'cookies' => $cookies, 'timeoute' => 20));
				if( is_wp_error($response ) ) {
					$this->messageHandler->http_error($url, $post_data, $response, __FILE__, __LINE__);
		...
		$this->messageHandler->notice('Another job well done (' . $action . '). <a href="' . $options['url'] . '/thread-' . $mtid . '.html">View Cross-Post</a><br/>');
	}
	...
	private function login() {
		...
		$response = wp_remote_post($url, array('body' => $post_data, 'timeout' => 20));
		if (is_wp_error($response)) {
			$this->messageHandler->http_error($url, $post_data, $response, __FILE__, __LINE__);
		...
		if (strpos($response['body'], $options['xp_login_ok']) === false) {
            $this->messageHandler->error("Unable to login", __FILE__, __LINE__);
			return;
		}
